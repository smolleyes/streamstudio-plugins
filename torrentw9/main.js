...
tw9.init = function (gui, win, doc, console) {
  ...
  $(doc).on("click", ".preload_tw9Play_torrent", function (e) {
    ...
    var html =
      ...
      obj.cover +
      '" /><br><br> \
	<button type="button" id="tw9_play_' +
      id +
      '" data="' +
      encodeURIComponent(JSON.stringify(obj)) +
      '" class="closePopup play_tw9_torrent btn btn-success"> \
    	<span class="glyphicon glyphicon-play-circle"><span class="fbxMsg_glyphText">' +
      _("Start playing") +
      '</span></span> \
    </button>  \
    <button type="button" class="closePopup download_tw9_torrentFile downloadText btn btn-info" href="' +
      obj.torrent +
      '" id="tw9_downlink_' +
      obj.id +
      '" data="' +
    ...
    if (tw9.gui.freeboxAvailable) {
      html +=
        ...
        obj.torrent +
        '" class="closePopup download_tw9_torrentFile_fbx downloadText btn btn-info" id="tw9_downlinkFbx_' +
        obj.id +
        '" data="' +
        encodeURIComponent(JSON.stringify(obj)) +
        '" title="' +
...
function loadEngine() {
  ...
  if ($.inArray(tw9.gui.settings.locale, localeList) > -1) {
    console.log(
      "Loading torrentw9 engine with locale" + tw9.gui.settings.locale
    );
  ...
  // orderBy filters and default entry
  tw9.category_filters = JSON.parse(
    '{"' +
      _("Movies") +
      '":"films","' +
      _("Movies DVDRIP") +
      '":"films DVDRIP (.avi)","' +
      _("Movies DVDRIP (x264)") +
      '":"films DVDRIP (x264)","' +
      _("Movies 1080p") +
      '":"films 1080p","' +
      _("Movies 720p") +
...
tw9.search = function (query, options, gui) {
  ...
  if (page == 0) {
    $("#items_container")
      .empty()
      .append('<ul id="tw9_cont" class="list"></ul>')
      .show();
    tw9.itemsCount = 0;
  }
  ...
  cloudscraper.get(url, function (error, response, data) {
    if (error) {
      ...
      tw9.totalPages =
        parseInt($("ul.pagination li:last", data).prev().text()) || "";
...
function analyseResults(list) {
  ...
  if (searchType === "navigation") {
    ...
    $("#search_results p")
      .empty()
      .append(_("%s results founds", tw9.itemsCount))
      .show();
  }
}
...
function appendVideo(video) {
  ...
  cloudscraper.request(
    { method: "GET", url: video.link, Referer: tw9.url },
    function (error, response, res) {
      ...
      //save in data
      $("#" + video.id + " .preload_tw9_torrent").attr(
        "data",
